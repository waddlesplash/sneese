...
#if !SCREEN_HIRES
static void CONCAT_6_NAME(Plot_Lines_, SCREEN_TYPE, _Offset_, OFFSET_TYPE, _C2_, PLOTTER_TYPE) (
 ...
 int tile_on_line_offset)
{
 do
 {
  ...
  /*  tile-lines for 2-bit tiles is 32k (64k / 16 * 8) */
   (64 * 1024 / 16 * 8) - 1,
   map & BGSC_FLIP_X, 1, line_increment, palette_mask,
   tile_clip_1_left, tile_clip_1_right, TileCache2);

 } while (output_surface_offset += 8 / DOT_WIDTH_DIVISOR,
  ++tile_on_line_offset, --tile_count);
}
#endif

static void CONCAT_7_NAME(Plot_Lines_, RES_SUFFIX_U, SCREEN_TYPE, _Offset_, OFFSET_TYPE, _C4_, PLOTTER_TYPE) (
 const void *offset_map_address_base, unsigned tileset_address,
 unsigned tile_line_address_current, unsigned tile_line_address_y_current,
 unsigned char (*main_buf)[2], unsigned char (*sub_buf)[2],
 unsigned output_surface_offset, int line_count, int tile_count,
...
/*if (oc_flag & 0x20)
  ...
  /*  tile-lines for 4-bit tiles is 16k (64k / 32 * 8) */
   (64 * 1024 / 32 * 8) - 1,
   map & BGSC_FLIP_X, 1, line_increment, palette_mask,
   tile_clip_1_left, tile_clip_1_right, TileCache4);
...
#if !SCREEN_HIRES
static void CONCAT_6_NAME(Plot_Lines_, SCREEN_TYPE, _Offset_, OFFSET_TYPE, _C8_, PLOTTER_TYPE) (
 const void *offset_map_address_base, unsigned tileset_address,
 unsigned tile_line_address_current, unsigned tile_line_address_y_current,
 unsigned char (*main_buf)[2], unsigned char (*sub_buf)[2],
 unsigned output_surface_offset, int line_count, int tile_count,
